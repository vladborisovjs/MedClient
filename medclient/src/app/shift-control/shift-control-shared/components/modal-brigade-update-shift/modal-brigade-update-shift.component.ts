...
export class ModalBrigadeUpdateShiftComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.shiftItem.id) { // если редактирование смены то нельзя редактировать
      ...
      this.shiftItem.dateFrom = new Date(this.date.getFullYear(), this.date.getMonth(), this.date.getDate(), 0);
      this.shiftItem.dateTo = new Date(this.date.getFullYear(), this.date.getMonth(), this.date.getDate(), 23, 59);
    }
