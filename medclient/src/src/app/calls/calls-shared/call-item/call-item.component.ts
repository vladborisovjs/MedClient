...
export class CallItemComponent implements OnInit, OnDestroy {
  ...
  isEdit: boolean;
  tryToUnlock: boolean = false;
  coordinates = [0,0];

  // если получаем токен блокировки
    ...
    this.sbscs.push(
      this.cs.callItemSub.subscribe(
        call => {
          console.log(call);
          this.callItem = call;
          const ukioCardS = localStorage.getItem('ukio'); // {performerId: number, sessionId: string, fromNumber: string, ukio: any}
          if (ukioCardS) {
            const ukioCard: { performerId: number, sessionId: string, fromNumber: string, ukio: any } = JSON.parse(ukioCardS);
  ...
  resetPatientList(disableForm = true) {
    ...
    this.callItem.call.patientList.forEach(
      (patient) => {
        ...
        const item = PatientTemplateBean.fromJS(patient);
        const subscription = form.valueChanges.subscribe(ch => Object.assign(item, ch));
        this.patients.push(
          {
            item: item,
            form: form,
  ...
  addPatient() {
    const item = PatientTemplateBean.fromJS({id: 0, isDeleted: false, call: this.callItem.call.id});
    const form = this.sds.makeForm(this.getBlockDescriptions('patient'));
