...
export class SocketTopicsService {
  ...
  currentOverdue: any; // текущий открытый звонок
      ...
      this.brigadeOverdueSub.pipe(filter(() => this.showSocketModals)).subscribe(
        (overdue: number[]) => {
          ...
          if (
            ...
            this.currentOverdue = true;
            if (!this.currentOverdue){
              const bo = this.modal.open(ModalBrigadeOverdueComponent, {backdrop: "static"});
              bo.result.then(
                res => {
                  if (res){
                    this.router.navigate(['/calls']);
                  }
                  this.currentOverdue = false;
                },
                ()=>{this.currentOverdue = false;}
              );
            }
