...
audioList: any[];
  ...
  getAudioLink(audio) {
    this.http.get('http://localhost:4200/api/call/audio/download/'+audio.id).pipe(take(1)).subscribe(
      audioLink => {
    ...
    // audio.link ='http://localhost:4200/api/call/audio/preview/'+audio.id;
