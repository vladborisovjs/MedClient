...
export class PatientsArchiveComponent implements OnInit {
  ...
  selectedPatient; // пациент для удаления
  Roles = _Roles;
  ...
  deletePatient() {
    this.arch.deletePatient(this.selectedPatient).pipe(take(1)).subscribe(
      res => {
  ...
  restorePatient() {
    this.arch.restorePatient(this.selectedPatient).pipe(take(1)).subscribe(
      res => {
        this.ns.success('Успешно', `Пациент восстановлен`);
        this.selectedPatient = null;
      }
    );
  }
